...
export const useAppMutate = () => {
  ...
  const createTaskMutation = useMutation(
    (title: string) => graphQLClient.request(CREATE_TASKS, { title }),
    {
      onSuccess: (res) => {
  ...
  const updateNewsMutation = useMutation(
    ...
    {
      onSuccess: (res, variables) => {
        const previousTodos = queryClient.getQueryData<News[]>('news')
        if (previousTodos) {
          queryClient.setQueryData<News[]>(
            'news',
    ...
    (id: string) => graphQLClient.request(DELETE_NEWS, { id }),
    {
      onSuccess: (res, variables) => {
        ...
        if (previousTodos) {
          queryClient.setQueryData<News[]>(
            'news',
            previousTodos.filter((news) => news.id !== variables)
          )
        }
        dispatch(resetEditedNews())
      },
